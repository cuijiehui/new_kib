 ...
 */
public class PayRecord extends Leaf {
     ...
     * @param content                  内容
     */
    public PayRecord(IBluetoothResultCallback iBluetoothResultCallback, int len, byte[] content) {
        super(iBluetoothResultCallback, BluetoothCommandConstant.COMMAND_CODE_PAY_RECORD, BluetoothCommandConstant.ACTION_CHECK);
...
//            int totalBytesSize = (int) ParseUtil.bytesToLong(contents, 1, 1);   // 总字节数
            int count = (int) ParseUtil.bytesToLong(contents, 1, 2);            // 记录条数
            int contentLen = (int) ParseUtil.bytesToLong(contents, 3, 3);       // 记录字节数
