 ...
 */
public class GetHeartRateDataEx extends Leaf {
     ...
     * 构造函数(0x70)
     *
     * @param iBluetoothResultCallback 蓝牙结果回调
     * @param len                      内容长度
     * @param content70                内容
     * @param heartRateCount           需要获取的心率条数
     */
    public GetHeartRateDataEx(IBluetoothResultCallback iBluetoothResultCallback, int len, int content70, int heartRateCount) {
        super(iBluetoothResultCallback, BluetoothCommandConstant.COMMAND_CODE_GET_HEART_RATE_DATA_EX, BluetoothCommandConstant.ACTION_CHECK);
    ...
    public int parse80BytesArray(int len, byte[] contents) {
        ...
        if (len > 0 && len % 7 == 0) {
            ...
            int heartRateValue = contents[6] & 0xFF;                                                // 心率值

            int index1 = (int) ParseUtil.bytesToLong(contents, 7, 8);                               // 索引值
            long timeStamp1 = ParseUtil.bytesToLong(contents, 9, 12);                               // 时间戳
            int heartRateValue1 = contents[13] & 0xFF;                                              // 心率值
            LogUtil.i(TAG, "查询返回 : 心率数据(索引值:" + index + " 时间(" + timeStamp + "):" + ParseUtil.eightTZTimeStampToStringTZ(timeStamp, false) + " 心率:" + heartRateValue);
            LogUtil.i(TAG, "查询返回 : 心率数据(索引值:" + index1 + " 时间(" + timeStamp1 + "):" + ParseUtil.eightTZTimeStampToStringTZ(timeStamp1, false) + " 心率:" + heartRateValue1);

            if (bluetoothVar.heartRateBTDataList == null || bluetoothVar.heartRateBTDataList.size() == 0 || index == 1 || index1 == 1) {
            ...
            bluetoothVar.heartRateBTDataList.add(new HeartRateBT(index1, heartRateValue1, timeStamp1));
